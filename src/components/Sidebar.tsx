...
import { Home, Settings, Building2, Cloud, Factory, Filter, Smartphone as Sim, Users, Database, ShoppingCart, UserCog, User, CreditCard, Shield, MessageSquare, ChevronDown, ChevronRight } from 'lucide-react';
...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { key: 'rechargeImport', path: '/user-management/recharge-import-regular' },
  ...
  { key: 'onlineSales', icon: ShoppingCart, children: [] },
  { key: 'employeeManagement', icon: UserCog, children: [] },
  { key: 'personalCenter', icon: User, children: [] },
  { key: 'paymentManagement', icon: CreditCard, children: [] },
  { key: 'advancedManagement', icon: Shield, children: [] },
  { key: 'smsManagement', icon: MessageSquare, children: [] }
];
  ...
  return (
    <>
      ...
      <div className={`
        ...
        w-64 bg-[#2C3E50] text-white h-screen overflow-y-auto flex flex-col
            ...
            {item.children ? (
              <>
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-[#34495E] rounded transition-colors"
                >
                  ...
                  <div className="ml-8 mt-1">
                    {item.children.map((child) => (
                      <Link
                        key={child.key}
                        to={child.path}
                        className={`block px-4 py-2 text-sm hover:bg-[#34495E] rounded transition-colors ${
                          isActive(child.path) ? 'bg-[#4A90E2]' : ''
                        }`}
                      >
                        {t(`nav.${child.key}`)}
                      </Link>
              ...
              <Link
                to={item.path || '#'}
                className={`flex items-center px-4 py-3 hover:bg-[#34495E] rounded transition-colors ${

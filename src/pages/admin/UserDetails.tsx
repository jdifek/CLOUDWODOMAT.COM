...
export function UserDetails() {
  ...
  const [vendingDevices, setVendingDevices] = useState<any[]>([]);
  const [vendingLoading, setVendingLoading] = useState(true);
  useEffect(() => {
    fetchSettings();
    fetchUser();
  }, [id]);
  const fetchVendingDevicesForUser = async (userData: any) => {
    ...
    try {
      ...
      const all = results.flatMap((res) =>
        res.data.code === 0 ? res.data.data : []
      );
      setVendingDevices(all);
    } catch (error) {
  ...
  const fetchUser = async () => {
    try {
      ...
      fetchVendingDevicesForUser(response.data);
    } catch {
  ...
  const calculatedDevicesCount = vendingDevices.length;
  ...
  const handleSaveSubscription = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        price: user.subscription
          ? parseFloat(subscriptionForm.price)
          : calculatedPrice,
        devicesCount: user.subscription
          ? parseInt(subscriptionForm.devicesCount)
          : calculatedDevicesCount,
        currentPeriodEnd: subscriptionForm.currentPeriodEnd
          ? new Date(subscriptionForm.currentPeriodEnd).toISOString()
          : null,
      };
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            ...
            {editMode ? (
              <input
                ...
                onChange={(e) =>
                  setEditData({ ...editData, appid: e.target.value })
                }
                className="w-full px-3 py-2 border rounded"
                placeholder="appid"
              />
          ...
          <div>
            ...
            {editMode ? (
              <input
                ...
                onChange={(e) =>
                  setEditData({ ...editData, saler: e.target.value })
                }
                className="w-full px-3 py-2 border rounded"
                placeholder="saler"
              />
        ...
        {user.subscription ? (
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-gray-900">
                zł{user.subscription.price.toFixed(2)}/month
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        ) : vendingDevices.length > 0 ? (
          <div className="space-y-2">
            {vendingDevices.map((device: any) => (
            ...
            <form onSubmit={handleAddDevice} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDeviceForm({ ...deviceForm, code: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#4A90E2]"
                  required
                />
              ...
              <div>
                ...
                {!user.subscription && (
                  <p className="text-xs text-gray-500 mt-1">
                    {t("admin.autoCalculated")}: zł{basePrice.toFixed(2)} ×{" "}
                    {calculatedDevicesCount} {t("admin.devices").toLowerCase()}
                  </p>
                )}
              </div>
              <div>
                ...
                <input
                  ...
                  value={
                    user.subscription
                      ? subscriptionForm.devicesCount
                      : calculatedDevicesCount
                  }
                  onChange={(e) =>
                    setSubscriptionForm({
                      ...subscriptionForm,
                      devicesCount: e.target.value,
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#4A90E2] bg-gray-50"
                  disabled={true}

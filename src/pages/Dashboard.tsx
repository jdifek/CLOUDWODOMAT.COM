...
export function Dashboard() {
  ...
  const prepareChartData = (records: any[]) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const revenue = dayRecords.reduce((sum, r) => 
        sum + parseFloat(r.value || "0"), 0
      );
      ...
      last7Days.push({
        ...
        subscriptions: Math.floor(sales * 0.8),
      });
    }
  ...
  const topDevicesByLocation = devices
    .slice(0, 5)
    .map((d, idx) => ({
      id: idx + 1,
      channel: d.location || "N/A",
      quantity: Math.floor(Math.random() * 200) + 50,
      type: (d.is_online === "online" || d.is_onlie === "online") ? "Online" : "Offline",
    }));
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie
                data={[
                  { name: "Poland", value: devices.length },
                  { name: "Other", value: 0 },
                ]}
                cx="50%"
                cy="50%"
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm">
        ...
        <div className="text-gray-600">
          {activeTab === "profit" && (
            <div>
              <p className="mb-2">
                <strong>Общая прибыль:</strong> {stats.currentRevenue.toFixed(2)} ¥
              </p>
              <p className="mb-2">
                <strong>Прибыль за сегодня:</strong> {stats.todayRevenue.toFixed(2)} ¥
              </p>
            </div>
          )}
          {activeTab === "sales" && (
          ...
          {activeTab === "exchange" && (
            <div>
              <p className="mb-2">
                <strong>Активных устройств:</strong> {deviceStats.online} / {deviceStats.total}
              </p>
            </div>
          )}
        </div>
